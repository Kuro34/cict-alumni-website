...
<head>
    ...
    <style>
        ...
        .resend-btn { background-color: gray; margin-top: 10px; }
...
<div class="container">
    ...
    <form action="{{ route('otp.verify') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="number" name="otp_code" id="otp_code" min="0" required>
            <input type="hidden" name="alumniID" value="{{ request('alumniID') }}">
        </div>
        <button type="submit" class="btn">Verify OTP</button>
    </form>

    <form method="POST" action="{{ route('otp.changeEmail') }}" style="margin-top: 10px;">
        @csrf
        <button type="submit" class="btn">Change Email</button>
    </form>

    <form action="{{ route('otp.resend') }}" method="POST" id="resendForm" style="margin-top: 10px;">
        @csrf
        <input type="hidden" name="alumniID" value="{{ request('alumniID') }}">
...
<script>
    let cooldown = {{ session('otp_cooldown', 30) }};
